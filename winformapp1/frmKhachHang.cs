...
namespace WinFormsApp2
{
    public partial class frmKhachHang : Form
    {
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            ...
            string sNgaySinh = dateTimePicker1.Value.ToString("yyyy-MM-dd");
            string iGioiTinh = rbNam.Checked ? "NAM" : (rbNu.Checked ? "NU" : null);
            string sMaPhong = txtMaPhong.Text;
            string sQueQuan = txtQueQuan.Text;
            if (string.IsNullOrEmpty(sMaKH) || string.IsNullOrEmpty(sTenKH) || string.IsNullOrEmpty(sQueQuan))
            {
                MessageBox.Show("Vui lòng nhập đầy đủ thông.", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                con.Close();
                return;
            }
